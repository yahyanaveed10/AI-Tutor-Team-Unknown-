...
def _maybe_save_audio(audio: bytes | None, name: str) -> None:
    """Optionally write generated mp3 bytes to disk for debugging."""
    if not settings.SAVE_TTS_AUDIO:
        return
    if not audio:
        return
...
def run_conversation(
    ...
    opener, opener_audio = llm.generate_opener(topic_name)
    log.info(f"[Turn 0] Tutor: {opener[:80]}...")
    _maybe_save_audio(opener_audio, f"{student_id}_{topic_id}_turn0_opener")
    ...
    while state.turn_count < turns and not response.get("is_complete"):
        ...
        if not level_frozen and state.confidence < 0.75:
            ...
            tutor_audio = analysis.next_message_audio
            log.info(f"[DIAGNOSIS Turn {state.turn_count}] Level={analysis.estimated_level} Conf={smoothed_conf:.2f} (raw={raw_conf:.2f})")

            _maybe_save_audio(tutor_audio, f"{student_id}_{topic_id}_turn{state.turn_count}_diagnosis")
            ...
            # TUTORING PHASE
            tutoring_started = True
            tutor_msg, tutor_audio = llm.tutor(state, student_msg)
            log.info(f"[TUTORING Turn {state.turn_count}] Teaching at Level {state.estimated_level}")

            _maybe_save_audio(tutor_audio, f"{student_id}_{topic_id}_turn{state.turn_count}_tutor")
    ...
    # Optional: final spoken feedback summary
    summary = llm.generate_summary(state)
    log.info(f"[SUMMARY] {summary.text}")
    _maybe_save_audio(summary.audio, f"{student_id}_{topic_id}_final_summary")
